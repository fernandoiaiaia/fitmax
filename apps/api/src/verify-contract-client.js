...
console.log('==================================================');
console.log('🔍 INICIANDO AUDITORIA DE CONTRATO PACT: API PROVIDER (WEB-CLIENT)');
console.log('==================================================');

const BASE_URL = process.env.BASE_URL || 'http://localhost:3001';
const contractPath = path.resolve(__dirname, '../../../pacts/web-client-contract.json');
...
function validarSchema(data, schema, pfx = '') {
  ...
  if (schema.type === 'array') {
    ...
    // Tratamento de tipos união (ex: type: ['string', 'number'])
    const valid = schema.type.some(t => {
      if (t === 'null' && data === null) return true;
      return typeof data === t;
    });
    if (!valid) {
      errors.push(`${pfx} deve ser um dos tipos: [${schema.type.join(', ')}] (recebeu: ${typeof data})`);
    }
  } else {
...
async function run() {
  ...
  for (const interaction of contract.interactions) {
    ...
    try {
      ...
      if (interaction.name === 'Login de Paciente') {
  ...
  console.log('\n==================================================');
  console.log('📊 RELATÓRIO CONSOLIDADO DE AUDITORIA DE CONTRATOS (WEB-CLIENT)');
  console.log('==================================================');
  report.forEach(item => {
    if (item.success) {
  ...
  if (testsFailed) {
    ...
    console.log('\n🎉 SUCESSO ABSOLUTO: Todos os contratos do paciente validados e sem quebras!');
    process.exit(0);
  }
}
