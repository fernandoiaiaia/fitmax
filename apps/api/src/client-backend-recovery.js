// apps/api/src/client-backend-recovery.js
// Global intercept fetch to inject the bypass header
const originalFetch = global.fetch;
global.fetch = function (url, options = {}) {
  options.headers = options.headers || {};
  if (typeof Headers !== 'undefined' && options.headers instanceof Headers) {
    options.headers.set('x-bypass-rate-limit', 'true');
  } else if (Array.isArray(options.headers)) {
    options.headers.push(['x-bypass-rate-limit', 'true']);
  } else {
...
console.log('==================================================');
console.log('🔥 INICIANDO ENGENHARIA DE CAOS & RECUPERAÇÃO DO PACIENTE (CLIENT)');
console.log('==================================================');
...
async function run() {
  ...
  try {
    ...
    const loginRes = await fetch(`${BASE_URL}/api/auth/client/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'carlos.mendes@email.com', password: 'Client@123' })
    });
    ...
    const listRes = await fetch(`${BASE_URL}/api/client-portal/consultas`, {
    ...
    console.log('   ... Simulação ativada: Qualquer comando no Redis falhará.');

    console.log('   - Testando Login de Paciente com Redis offline (Bypass de Lockout)...');
    const loginChaosStart = Date.now();
    const loginChaosRes = await fetch(`${BASE_URL}/api/auth/client/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'carlos.mendes@email.com', password: 'Client@123' })
    });
    ...
    console.log('   - Testando rota de consultas com Redis offline...');
    const listChaosStart = Date.now();
    const listChaosRes = await fetch(`${BASE_URL}/api/client-portal/consultas`, {
      headers: { 'Authorization': `Bearer ${accessToken}` }
    });
    ...
    console.log('   ... Simulação ativada: Rotas atrasadas artificialmente em 2500ms.');
    ...
    try {
      await fetch(`${BASE_URL}/api/client-portal/consultas`, {
        headers: { 'Authorization': `Bearer ${accessToken}` },
    ...
    console.log('   ... Restauração ativada: Todos os gargalos e falhas limpos do ecossistema.');

    // Aguardar regeneração
    await sleep(500);

    const healStart = Date.now();
    const healRes = await fetch(`${BASE_URL}/api/client-portal/consultas`, {
      headers: { 'Authorization': `Bearer ${accessToken}` }
    });
  ...
  // -------------------------------------------------------------------------
  console.log('\n==================================================');
  console.log('📊 BALANÇO CONSOLIDADO DE ENGENHARIA DE CAOS (CLIENT)');
  console.log('==================================================');
  report.forEach(item => {
    if (item.success) {
  ...
  if (testsFailed) {
    console.error(`\n${RED}${BOLD}❌ FALHA: A resiliência do backend do paciente falhou em cenários de caos!${RESET}`);
    process.exit(1);
  } else {
    console.log(`\n${GREEN}${BOLD}🎉 SUCESSO ABSOLUTO: Backend do paciente resiliente com Graceful Degradation e Self-Healing!${RESET}`);
    process.exit(0);
  }
}
