// apps/api/src/pro-backend-scalability.js
// Teste de estresse e escalabilidade portátil para o módulo profissional do FITMAX API
...
async function authenticate() {
  console.log(`🔐 Autenticando Profissional em ${BASE_URL}/api/auth/pro/login...`);
  ...
  const response = await fetch(`${BASE_URL}/api/auth/pro/login`, {
    method: 'POST',
    headers,
    body: JSON.stringify({
      email: PRO_EMAIL,
  ...
  async function worker() {
    ...
    while (true) {
      ...
      try {
        ...
        const response = await fetch(`${BASE_URL}/api/pro/consultas?limit=50`, {
          method: 'GET',
          headers,
        });
...
function reportMetrics(metrics) {
  ...
  console.log(`\n==================================================`);
  console.log(`📊 SUMÁRIO DE ESTRESSE DO BACKEND DO PROFISSIONAL`);
  console.log(`==================================================`);
