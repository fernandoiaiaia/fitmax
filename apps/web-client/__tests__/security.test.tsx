...
describe('Front-End OWASP Security Test Suite - web-client', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    tokenStore.clear();
    localStorage.clear();
  ...
  it('OWASP A07 (Generic Login Errors): Displays the identical secure generic error banner on login failures, preventing username enumeration', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Credenciais inválidas. Verifique e tente novamente.')).toBeDefined();
    });
    ...
    fireEvent.change(screen.getByLabelText(/E-mail/i), { target: { value: 'carlos.mendes@email.com' } });
  ...
  it('OWASP A07 (Rate Limiting handling): Properly alerts the user when API rate limit threshold is hit (HTTP 429)', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Muitas tentativas. Tente novamente em 15 minutos.')).toBeDefined();
    });
  });
