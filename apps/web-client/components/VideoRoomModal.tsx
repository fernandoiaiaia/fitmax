      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-5xl h-[85vh] bg-[#141414] border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col relative"
          >
            {/* Header */}
            <div className="absolute top-0 w-full flex items-center justify-between px-6 py-4 bg-gradient-to-b from-black/80 to-transparent z-50">
              <div>
                <h2 className="text-white text-lg font-bold flex items-center gap-2 drop-shadow-md">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_#10b981] animate-pulse" />
                  Consulta Online
                </h2>
                <p className="text-zinc-300 text-xs mt-1 drop-shadow-sm">Sala: {channelName}</p>
              </div>
              <button
                onClick={onClose}
                className="w-10 h-10 rounded-full bg-black/40 flex items-center justify-center text-zinc-300 hover:text-white hover:bg-red-500/80 transition-all backdrop-blur-md"
                title="Sair da sala"
              >
      ...
      <div className="flex-1 w-full h-full relative flex items-center justify-center bg-zinc-950">
        {mainRemoteUser ? (
          ...
          <div className="flex flex-col items-center justify-center text-zinc-500">
            ...
            <p className="text-lg">Aguardando o profissional iniciar o atendimento...</p>
          </div>
        )}
      </div>

      {/* Local Video PIP */}
      <div className="absolute top-20 right-6 w-32 h-44 sm:w-40 sm:h-56 bg-zinc-900 border-2 border-zinc-700/50 rounded-2xl overflow-hidden shadow-2xl z-10 transition-transform hover:scale-105">
        {localCameraTrack ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-zinc-900 text-zinc-600 text-xs">Câmera Off</div>
        )}
